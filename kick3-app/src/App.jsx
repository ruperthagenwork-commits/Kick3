...
// ============ TOURNAMENT MODE — FOUNDATIONS ============ // Pure plumbing. No UI yet. Defines: //   - When the tournament runs (11 June – 19 July 2026, 13 synchronised 3-day cycles) //   - getTournamentCycle(date) — given a date, returns which cycle/day is live (or null if outside window) //   - Beta gate (?beta=pete) — hides tournament UI from anyone without the flag, even after merge to main //   - Debug day override (?debug=tournament-day-1/2/3) — for testing all three days in one sitting //   - readTournamentState / writeTournamentState — localStorage helpers, namespace kick3_tournament_v1 // This block produces zero visible change. It only defines functions and reads from localStorage.  const TOURNAMENT_CONFIG = {   // World Cup window. Cycles tick over at UK midnight, same as the daily question.   startDate: '2026-06-11',  // Day 1 of Cycle 1   endDate:   '2026-07-19',  // Last day of the tournament window (World Cup final day)   cycleLengthDays: 3,   // Opponent for each day in a cycle (index 0 = Day 1).   opponents: ['Pub Mate', 'Statistician', 'Pete the Pundit'],   // localStorage namespace. Versioned so we can migrate cleanly later if the shape changes.   storageKey: 'kick3_tournament_v1',   // Beta flag — when present in URL or localStorage, tournament UI is visible.   // Used during build to hide tournament mode from the public after we eventually merge to main.   // For now the branch isolation handles hiding anyway; this is belt-and-braces for launch week.   betaParam: 'pete', };  // Parse 'YYYY-MM-DD' into a Date at UK midnight (treating the date as a calendar day in UK time). // We use UTC noon as the anchor so DST transitions don't push us across a day boundary. const parseTournamentDate = (str) => {   const [y, m, d] = str.split('-').map(Number);   return new Date(Date.UTC(y, m - 1, d, 12, 0, 0)); };  // Days between two dates (calendar days, ignoring time-of-day). const daysBetween = (from, to) => {   const ms = to.getTime() - from.getTime();   return Math.floor(ms / (1000 * 60 * 60 * 24)); };  // Given a Date, return tournament status. Returns null if outside the tournament window. // Inside the window, returns { cycleNumber, dayInCycle, opponent, totalCycles }. //   cycleNumber: 1-based (Cycle 1 starts on TOURNAMENT_CONFIG.startDate) //   dayInCycle:  1, 2, or 3 //   opponent:    string name of today's opponent //   totalCycles: total cycles in the window (derived, currently 13) const getTournamentCycle = (date) => {   const start = parseTournamentDate(TOURNAMENT_CONFIG.startDate);   const end   = parseTournamentDate(TOURNAMENT_CONFIG.endDate);   // Snap input date to noon UTC for the same DST-safe comparison.   const dayAnchor = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 12, 0, 0));   if (dayAnchor < start || dayAnchor > end) return null;   const offset = daysBetween(start, dayAnchor);  // 0 on Day 1 of Cycle 1   const cycleNumber = Math.floor(offset / TOURNAMENT_CONFIG.cycleLengthDays) + 1;   const dayInCycle  = (offset % TOURNAMENT_CONFIG.cycleLengthDays) + 1;   const totalCycles = Math.ceil((daysBetween(start, end) + 1) / TOURNAMENT_CONFIG.cycleLengthDays);   return {     cycleNumber,     dayInCycle,     opponent: TOURNAMENT_CONFIG.opponents[dayInCycle - 1],     totalCycles,   }; };  // Read beta flag from URL (?beta=pete) or localStorage. // On first visit with the URL param, persist to localStorage so the flag survives navigation. // To turn off: visit kick3.app?beta=off (clears the flag). const isTournamentBetaActive = () => {   try {     const params = new URLSearchParams(window.location.search);     const betaParam = params.get('beta');     if (betaParam === 'off') {       localStorage.removeItem('kick3_tournament_beta');       return false;     }     if (betaParam === TOURNAMENT_CONFIG.betaParam) {       localStorage.setItem('kick3_tournament_beta', '1');       return true;     }     return localStorage.getItem('kick3_tournament_beta') === '1';   } catch {     return false;   } };  // Debug day override — lets us test all three rounds in one sitting without waiting for real days. // Usage: kick3.app?debug=tournament-day-1   (or -day-2, -day-3) // Returns 1, 2, 3, or null if no override is set. const getTournamentDebugDay = () => {   try {     const params = new URLSearchParams(window.location.search);     const debug = params.get('debug');     if (debug === 'tournament-day-1') return 1;     if (debug === 'tournament-day-2') return 2;     if (debug === 'tournament-day-3') return 3;     return null;   } catch {     return null;   } };  // Default tournament state for a player who has never played a tournament before. const defaultTournamentState = () => ({   cycleNumber: null,        // Which cycle they're currently in (null = haven't started one)   dayInCycle: null,         // 1, 2, or 3   lastPlayedDate: null,     // ISO date string of last completed round   cycleResults: [],         // [{ day: 1, won: true, playerTotal: 26, opponentTotal: 19 }, ...]   trophyCount: 0,           // Lifetime trophies (wins against Pete)   tournamentsAttempted: 0,  // Lifetime count of cycles entered   status: 'idle',           // 'idle' | 'in-progress' | 'won-cycle' | 'lost-cycle' });  // Read tournament state from localStorage. Falls back to defaults on any failure // (private mode, disabled storage, corrupted JSON). const readTournamentState = () => {   try {     const raw = localStorage.getItem(TOURNAMENT_CONFIG.storageKey);     if (!raw) return defaultTournamentState();     const parsed = JSON.parse(raw);     if (!parsed || typeof parsed !== 'object') return defaultTournamentState();     // Merge with defaults so any missing fields get sensible values.     return { ...defaultTournamentState(), ...parsed };   } catch {     return defaultTournamentState();   } };  // Write tournament state to localStorage. Silent on failure — game still works without persistence. const writeTournamentState = (state) => {   try {     localStorage.setItem(TOURNAMENT_CONFIG.storageKey, JSON.stringify(state));   } catch { /* silent */ } };  // ============ END TOURNAMENT MODE — FOUNDATIONS ============
